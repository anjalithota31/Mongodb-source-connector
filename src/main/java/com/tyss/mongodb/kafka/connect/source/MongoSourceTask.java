      ...
      if (sourceConfig.isPartitionRotationEnabled()) {
        ...
        try {
          ...
          if (!adminProps.containsKey("bootstrap.servers")) {
            ...
            if (envBootstrapServers != null && !envBootstrapServers.isEmpty()) {
              adminProps.put("bootstrap.servers", envBootstrapServers);
              LOGGER.info(
                  "Using bootstrap.servers from environment variable CONNECT_BOOTSTRAP_SERVERS: {}",
                  envBootstrapServers);
            } else {
              LOGGER.warn(
